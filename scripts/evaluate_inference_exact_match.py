#!/usr/bin/env python3
"""
Evaluate inference results using accuracy evaluator with exact match comparator.

This script:
1. Reads JSON result files from an inference results directory
2. Evaluates each result using AccuracyEvaluator with ExactMatchComparator
3. Outputs detailed statistics and a CSV file with comparison results
"""
...
def create_comparator(comparator_name: str) -> BaseComparator:
    ...
    Args:
        comparator_name: Name of the comparator (e.g., "exact_match")
    ...
    if comparator_name_lower in ["exactmatch", "exact_match"]:
        return ExactMatchComparator()
    else:
        raise ValueError(
            f"Unknown comparator: {comparator_name}. "
            f"Available comparators: exact_match"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate inference results using accuracy evaluator with exact match comparator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Evaluate results from seephys_qwen3-vl (default: exact_match comparator, accuracy evaluator)
  python scripts/evaluate_inference_results.py \\
      --results-dir results/inference/seephys_qwen3-vl \\
      --output-dir results/evaluation/seephys_qwen3-vl
  ...
  python scripts/evaluate_inference_results.py \\
    ...
    parser.add_argument(
        ...
        type=str,
        default="exact_match",
        help="Comparator to use for answer comparison (default: exact_match). "
    ...
    logger.info("=" * 60)
    logger.info("Evaluate Inference Results")
    logger.info("=" * 60)
