...
- **Gemini now gates native structured output on schema compatibility.** Its plan hook previously inspected nothing and claimed native enforcement unconditionally, so a schema Gemini rejects became a hard 400 per request — amplified by the batch size. The gate rests on `google.genai.types.JSONSchema` — the pinned SDK's own model of the schema subset Gemini accepts — so only validation keywords that field set does not declare are gated, and a test fails if the two ever disagree. Google's *prose* allowlist is stricter than both: applied literally it would demote 32 of prkit's own 44 response models (it omits `default`, which the SDK declares), 23 (it forbids a `$ref` carrying any non-`$` sibling, which pydantic emits routinely) and 13 (cycles through required properties) — all sent natively today and working. Note Google now marks `responseJsonSchema` deprecated and has moved its structured-output guide to an API surface the pinned SDK does not expose.
