...
- **Gemini now gates native structured output on schema compatibility.** Its plan hook previously inspected nothing and claimed native enforcement unconditionally, so a schema Gemini rejects became a hard 400 per request — amplified by the batch size. The gate is deliberately narrow: Google's published allowlist for `response_json_schema`, applied literally, would demote 32 of prkit's own 44 response models (it omits `default`), 23 (it forbids a `$ref` carrying any non-`$` sibling, which pydantic emits routinely) and 13 (cycles through required properties) — all of which are sent natively today and work. Only constructs that carry validation semantics *and* appear in no prkit schema are gated. Note Google now marks `responseJsonSchema` deprecated and has moved its structured-output guide to an API surface the pinned SDK does not expose.
