...
from prkit.datasets.downloaders import PhyXDownloader
...
class TestPhyXDownloader:
    ...
    def test_do_download_invalid_split(self, temp_dir):
        ...
        with patch("datasets.load_dataset") as mock_load:
            ...
            with pytest.raises((ValueError, RuntimeError)):
                # Accessing protected method for testing purposes
                downloader._do_download(
                    download_dir, split="invalid_split"
                )  # pylint: disable=protected-access
    ...
    def test_process_rows_for_json_with_images(self, mock_pil_module, temp_dir):
        ...
        # Accessing protected method for testing purposes
        processed = downloader._process_rows_for_json(
            rows, download_dir
        )  # pylint: disable=protected-access
    ...
    def test_make_json_serializable(self):
        ...
        # Accessing protected method for testing purposes
        assert (
            downloader._make_json_serializable(None) is None
        )  # pylint: disable=protected-access
        assert (
            downloader._make_json_serializable("string") == "string"
        )  # pylint: disable=protected-access
        assert (
            downloader._make_json_serializable(42) == 42
        )  # pylint: disable=protected-access
        assert (
            downloader._make_json_serializable(3.14) == 3.14
        )  # pylint: disable=protected-access
        assert (
            downloader._make_json_serializable(True) is True
        )  # pylint: disable=protected-access

        # Test with dict
        result = downloader._make_json_serializable(
            {"key": "value"}
        )  # pylint: disable=protected-access
