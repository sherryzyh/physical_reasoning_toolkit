...
from physkit.physkit_core.models import Answer
from physkit.physkit_core.definitions import AnswerType
from physkit.physkit_evaluation.comparison import (
    NumericalComparator,
    SymbolicComparator,
    TextualComparator,
    OptionComparator,
...
class TestTextualComparator:
    ...
    @patch('physkit.physkit_evaluation.comparison.textual.LLMClient')
...
class TestOptionComparator:
    ...
    def test_compare_with_tolerance(self):
        ...
        answer1 = Answer(value="a", answer_type=AnswerType.OPTION)
        answer2 = Answer(value="A", answer_type=AnswerType.OPTION)
        result = comparator.compare_with_tolerance(answer1, answer2, case_sensitive=False)
        assert result["is_equal"] is True
...
class TestSmartAnswerComparator:
    ...
    def test_compare_different_types(self):
        ...
        assert result["details"].get("routing_method") == "fallback" or "is_equal" in result
    ...
    def test_smart_type_conversion_string_to_answer(self):
        ...
        assert converted2.answer_type == AnswerType.NUMERICAL  # Should match answer1's type
