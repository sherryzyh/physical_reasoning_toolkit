#!/usr/bin/env python3
"""Generate reference-semantics JSON artifacts for a dataset."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate reference semantics for a dataset and save one JSON artifact per problem.",
    )
    ...
    for index, problem in enumerate(dataset, start=1):
        logger.info(
            "[%s/%s] Generating reference semantics for %s",
            index,
            len(dataset),
            problem.problem_id,
        )
    ...
    manifest = {
        "artifact_type": "reference_semantics_manifest",
        "dataset_name": args.dataset,
        "variant": args.variant,
        "split": args.split,
        "model_name": args.model,
    ...
    logger.info("Saved reference semantics to %s", output_dir)
