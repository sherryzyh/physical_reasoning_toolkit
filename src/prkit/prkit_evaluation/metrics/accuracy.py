...
class AccuracyMetric(BaseMetric):
    ...
    def __init__(self, comparator: Optional[SmartAnswerComparator] = None):
        ...
        Args:
            comparator: SmartAnswerComparator instance to use for comparisons.
                       If None, creates a new instance.
        """
        super().__init__("Accuracy", "Measures the proportion of correct predictions")
    ...
    def compute_single(
        self, prediction: Answer, ground_truth: Answer
    ) -> Dict[str, Any]:
        """
    ...
    def get_accuracy_by_type(
        self, predictions: List[Answer], ground_truths: List[Answer]
        ...
        for pred, gt in zip(predictions, ground_truths):
            ...
            if answer_type not in type_groups:
                type_groups[answer_type] = {"predictions": [], "ground_truths": []}
        ...
        for answer_type, group in type_groups.items():
            if group["predictions"]:  # Only compute if we have samples
                results[answer_type] = self.compute(
                    group["predictions"], group["ground_truths"]
                )
