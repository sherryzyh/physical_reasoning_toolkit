...
class SmartAnswerComparator:
    ...
    def compare(
        ...
        return self._create_failure_result(
            answer1, answer2, primary_result, fallback_result
        )
            ...
            "details": {
                ...
                "answer_info": {
                    "answer1": {
                        ...
                        "value_preview": (
                            str(getattr(answer1, "value", "N/A"))[:50] + "..."
                    ...
                    "answer2": {
                        ...
                        "value_preview": (
                            str(getattr(answer2, "value", "N/A"))[:50] + "..."
    ...
    def diagnose(self, answer1: Answer, answer2: Answer) -> Dict[str, Any]:
        ...
        if (
            ...
            and getattr(answer1, "metadata", {}).get("conversion_method")
            == "automatic_string_to_textual"
        ):
        ...
        if (
            ...
            and getattr(answer2, "metadata", {}).get("conversion_method")
            == "automatic_string_to_textual"
        ):
            diagnosis["smart_conversion_applied"] = True
        ...
        if diagnosis["input_validation"]["valid"]:
            ...
            if types_match:
                if answer1.answer_type in self.comparators:
                    ...
                    diagnosis["recommendations"].append(
                        "❌ No comparator for this type"
                    )
            else:
                diagnosis["recommendations"].append(
                    "🔄 Types don't match - will use fallback with Answer(answer_type=TEXTUAL) conversion"
                )

            diagnosis["recommendations"].append(
                "🔄 Fallback mode always enabled - converts to Answer(answer_type=TEXTUAL) for cross-type comparison"
            )
            diagnosis["recommendations"].append(
                "🧠 Smart type conversion automatically handles mixed Answer/string inputs"
            )
