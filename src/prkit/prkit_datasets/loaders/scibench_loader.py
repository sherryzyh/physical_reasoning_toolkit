...
The loader automatically determines problem types, maps SciBench fields to standard PRKit fields,
...
class SciBenchLoader(BaseDatasetLoader):
    ...
    def DOMAIN_SUBJECT_MAPPING(self) -> Dict[str, str]:
        ...
        return {
            ...
            "stat": {"domain": "Statistics", "subject": "math", "problem_count": 72},
            "calculus": {"domain": "Calculus", "subject": "math", "problem_count": 42},
        }
    ...
    def field_mapping(self) -> Dict[str, str]:
        """
        Define field mapping from SciBench fields to standard PRKit fields.
    ...
    def _determine_answer_and_type(
        self, answer_latex: str, answer_number: str, unit: str
    ) -> Tuple[str, str, str]:
        ...
        if exponent:
            try:
                ...
                answer_value = (
                    f"{answer_number}x{unit}"
                    if answer_number
                    else f"{answer_latex}x{unit}"
                )
        ...
        if (
            answer_number
            and answer_number.strip() != ""
            and answer_latex
            and answer_latex.strip() != ""
            and answer_latex.strip() == answer_number.strip()
        ):
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for item in data:
                    if not isinstance(item, dict):
                        raise ValueError(
                            f"Expected item to be a dict, got {type(item)}: {item}"
                        )
                return data
    ...
    def _merge_problems_with_solutions(
        self, problems: List[Dict[str, Any]], solutions: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _process_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        answer_value, answer_type, extracted_unit = self._determine_answer_and_type(
            answer_latex, answer_number, unit
        )
        ...
        if answer_type == AnswerType.NUMERICAL and extracted_unit:
            metadata["answer"] = {"value": answer_value, "unit": extracted_unit}
        else:
            metadata["answer"] = answer_value
    ...
    def load(
        ...
        **kwargs,
        ...
        self.logger.debug(
            "Found %d problem files and %d solution files",
            len(problem_files),
            len(solution_files),
        )
        ...
        for idx, problem_data in enumerate(all_problems):
            try:
                ...
                self.logger.warning(
                    "Could not create problem from %s: %s",
                    problem_data.get("problemid", "unknown"),
                    e,
                )
                continue

        self.logger.debug(
            "Successfully created %d PhysicsProblem objects", len(physics_problems)
        )
        ...
        dataset = PhysicalDataset(
            ...
            info={
                ...
                "subjects_included": list(
                    set(p.domain.value for p in physics_problems if p.domain)
                ),
        ...
        self.logger.info(
            f"Successfully loaded {len(physics_problems)} problems from SciBench dataset"
        )
