"""
PhyBench LaTeX Preprocessing Script

This file contains LaTeX preprocessing functions for converting LaTeX mathematical
expressions to SymPy expressions. It is based on the original PhyBench repository.

Original Source: https://github.com/phybench-official/phybench/blob/main/EED/latex_pre_process.py
Repository: https://github.com/phybench-official/phybench
License: Please refer to the original PhyBench repository for licensing information.

This file is used to pre-process input latex expressions.
You only need a "master_convert()" function for basic usage.
"""
...
# Try to import timeout_decorator, but handle gracefully if not available
try:
    import timeout_decorator
    HAS_TIMEOUT_DECORATOR = True
except ImportError:
    HAS_TIMEOUT_DECORATOR = False
    print("Warning: timeout_decorator not available, using regular simplify")
...
def vec_lower_idx(input_str):
    ...
    Return：
        str(str): Converted
    """
    # Check if input_str is None or not a string
...
def first_pre_process(s,extrac_box=True):
    """
    First stage preprocessing of LaTeX expressions.
    """
    # Check if s is None or not a string
    if s is None:
        return None
...
def master_convert(s):
    ...
    try:
        ...
        if preprocessed_stage2 is None:
            return None

        # Use the correct function name from latex2sympy2_extended
        try:
            Sym = latex2sympy(preprocessed_stage2, normalization_config=MyNormalization(), conversion_config=MyConfig())
        except NameError:
            # Fallback if latex2sympy is not available
            print("Warning: latex2sympy function not available from latex2sympy2_extended")
            return None
        return Sym
