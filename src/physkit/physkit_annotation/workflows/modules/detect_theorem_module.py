...
class DetectTheoremModule(BaseWorkflowModule):
        ...
        **kwargs
        ...
        try:
            ...
            if not theorem_result:
                self.logger.warning(
                    f"Theorem detection result: {theorem_result}"
                )
            ...
            if hasattr(theorem_result, 'theorems') and theorem_result.theorems:
                ...
                total_problems = self.module_status["problems_with_theorems"] + self.module_status["problems_without_theorems"]
                if total_problems > 0:
            ...
            result = {
                ...
                "problem_id": problem_id,
                "question": question,
                "theorems": theorem_detection_dict.get("theorems") if isinstance(theorem_detection_dict, dict) else None,
                "metadata": {
                    "module_name": self.name,
                    "model_used": self.model,
                    "detection_type": "theorem",
                    "timestamp": self.module_status.get("metadata", {}).get("start_time", None)
                }
            }
            ...
            self.logger.error("Theorem detection failed for problem %s: %s", problem_id, str(e))
            self.module_status["failed_problems"] += 1
            return {
                "status": "FAILED",
                "error": str(e),
    ...
    def _form_output_as_a_problem(
        ...
        self.logger.info(f"_form_output_as_a_problem called with result type: {type(result)}")
        ...
        if isinstance(problem, PhysicsProblem):
            ...
            if hasattr(new_problem, 'additional_fields') and new_problem.additional_fields is not None:
                new_problem.additional_fields["theorems"] = result.get("theorems")
                new_problem.additional_fields["theorem_detection_metadata"] = result.get("metadata")
            else:
